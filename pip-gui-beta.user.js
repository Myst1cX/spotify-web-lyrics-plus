...
// (instead of collapsing that button slot into just a close control) when the page has told it
// what "play"/"pause" mean via the MediaSession API. This script never called
// navigator.mediaSession.setActionHandler() anywhere, so Chromium had nothing to bind that button
// to - tapping what looked like play/pause was actually landing on the close hit-target, closing
// the window instead. Same root cause explains why no seek/skip buttons ever appeared: Chromium
// only draws extra PiP buttons for actions the page has explicitly registered.
// Fix: initPipElements() now registers navigator.mediaSession.setActionHandler() for 'play' and
// 'pause' (forwarding to the existing handlePipVideoPlay()/handlePipVideoPause(), unchanged), plus
// 'previoustrack'/'nexttrack' (forwarding to the existing findSpotifyPreviousButton()/
// findSpotifyNextButton() finders). syncPipMediaStateFromSpotify() also sets
// navigator.mediaSession.playbackState alongside its existing play/pause/volume sync, so the PiP
// window's icon can't drift out of sync with the state actually driving pipVideo's native
// play/pause DOM events. Deliberately left out seekto/seekforward/seekbackward - pipVideo mirrors
// Spotify's own playback position rather than owning a real seekable timeline, so a seek handler
// would either have to drive Spotify's seek bar via DOM (another selector to keep in sync) or be a
// no-op button; previoustrack/nexttrack already cover the transport actions Spotify exposes as
// real buttons. No polling added, no functions relocated - registration lives right next to the
// pipVideo 'play'/'pause'/'volumechange' listeners it was missing beside.
...
document.head.appendChild(buttonGroupScrollStyle);
  ...
  function syncPipMediaStateFromSpotify() {
    ...
    try {
      ...
      if ('mediaSession' in navigator) {
        navigator.mediaSession.playbackState = spotifyPlaying ? 'playing' : 'paused';
      }
    } finally {
  ...
  async function initPipElements() {
    ...
    // MediaSession: pipVideo's source is canvas.captureStream(), which reports
    // duration=Infinity (treated as a "live" source). Chromium's native PiP
    // window only draws a real play/pause toggle (instead of collapsing that
    // slot into just a close control) - and only draws prev/next track buttons
    // at all - when the page has registered the corresponding MediaSession
    // action handlers. Without this, tapping "play/pause" in PiP was hitting
    // the close hit-target instead. Reuses the existing click-forwarding
    // handlers/finders below; no new control logic.
    if ('mediaSession' in navigator) {
      navigator.mediaSession.setActionHandler('play', () => handlePipVideoPlay());
      navigator.mediaSession.setActionHandler('pause', () => handlePipVideoPause());
      navigator.mediaSession.setActionHandler('previoustrack', () => {
        findSpotifyPreviousButton()?.click();
      });
      navigator.mediaSession.setActionHandler('nexttrack', () => {
        findSpotifyNextButton()?.click();
      });
    }
  }
