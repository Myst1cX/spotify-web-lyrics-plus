// ==UserScript==
// @name         Spotify Lyrics+ Beta Mobile
...
// 1. On Firefox for Android there is genuinely no native PiP API to call yet
//    (Mozilla's own PiP shipping notes say Android needs a separate native
//    implementation that hasn't landed) - the 17.37 fix below gets the
//    button working there via an in-page floating window, but it's confined
//    to the tab (no OS-level floating window, no surviving backgrounding).

// RESOLVED (17.37.beta): PIP TOGGLE NOW ACTUALLY WORKS ON BROWSERS WITHOUT A NATIVE PIP API (E.G. FIREFOX ANDROID)
// Two separate bugs, both on the "no native PiP" fallback path:
// 1. togglePip() called pipVideo.requestPictureInPicture() whenever the
//    function existed on the element - but Firefox appears to expose that
//    function on every platform, including Android, where the browser has
//    no native PiP implementation and the call simply rejects with
//    NotSupportedError. That rejection was caught by the *outer* try/catch,
//    which logged an error and returned - the page-PiP fallback code a few
//    lines below never ran at all. Fix: the requestPictureInPicture() call
//    now has its own try/catch; a rejection there just falls through to the
//    webkit tier and then the page-PiP tier instead of aborting the toggle.
// 2. Even when the page-PiP fallback did run, it only set isPagePipActive
//    and dispatched a synthetic 'enterpictureinpicture' event - which hid
//    the lyrics container and showed a "Playing in Picture-in-Picture" text
//    notice, but pipVideo/pipCanvas stayed hidden off-screen the whole time
//    (applyHiddenPipVideoStyle keeps it at -9999px/1x1px/opacity 0). There
//    was no actual floating window anywhere - the notice was describing a
//    PiP session that didn't visually exist.
//    Fix: new showPagePipWindow()/hidePagePipWindow() helpers build a small
//    draggable floating panel (fixed position, close button) and move the
//    already-live pipCanvas (the same canvas drawPipFrame() draws into for
//    real PiP) into it. togglePip()'s fallback branch now calls
//    showPagePipWindow() after dispatching the synthetic event; closePip()
//    calls hidePagePipWindow() during its synchronous cleanup branch. This
//    is still page-confined (no OS-level window, doesn't survive
//    backgrounding/switching apps) since there's no real API underneath it
//    on this platform - but there is now an actual visible, draggable
//    mini-player instead of a placeholder describing an invisible one.
...
document.head.appendChild(buttonGroupScrollStyle);
  ...
  // =============================================
  // "Page PiP" fallback window
  // =============================================
  // Used on browsers that expose neither requestPictureInPicture() nor
  // webkitSetPresentationMode() (currently: Firefox for Android). There is no
  // real browser API to fall back to here, so instead of leaving pipVideo
  // hidden off-screen with nothing visible, we show pipCanvas itself (the
  // same canvas drawPipFrame() already draws into for real PiP) inside a
  // small draggable floating panel positioned over the page. It's confined
  // to this tab - it won't float over other apps or survive backgrounding -
  // but it gives mobile Firefox users an actual floating mini-player instead
  // of just a "Playing in Picture-in-Picture" placeholder with nothing to show.
  let pagePipWindow = null;
  let pagePipDragState = null;

  function clampPagePipToViewport(win) {
    const rect = win.getBoundingClientRect();
    const maxLeft = Math.max(0, window.innerWidth - rect.width);
    const maxTop = Math.max(0, window.innerHeight - rect.height);
    const left = Math.min(Math.max(parseFloat(win.style.left) || 0, 0), maxLeft);
    const top = Math.min(Math.max(parseFloat(win.style.top) || 0, 0), maxTop);
    win.style.left = `${left}px`;
    win.style.top = `${top}px`;
  }

  function startPagePipDrag(clientX, clientY) {
    if (!pagePipWindow) return;
    const rect = pagePipWindow.getBoundingClientRect();
    pagePipDragState = { offsetX: clientX - rect.left, offsetY: clientY - rect.top };
  }

  function movePagePipDrag(clientX, clientY) {
    if (!pagePipWindow || !pagePipDragState) return;
    pagePipWindow.style.left = `${clientX - pagePipDragState.offsetX}px`;
    pagePipWindow.style.top = `${clientY - pagePipDragState.offsetY}px`;
    pagePipWindow.style.right = 'auto';
    pagePipWindow.style.bottom = 'auto';
    clampPagePipToViewport(pagePipWindow);
  }

  function endPagePipDrag() {
    pagePipDragState = null;
  }

  function showPagePipWindow() {
    if (!pipCanvas) return;
    if (pagePipWindow) { pagePipWindow.style.display = 'flex'; return; }

    const win = document.createElement('div');
    win.id = 'lyrics-plus-page-pip-window';
    ...
    closeBtn.setAttribute('aria-label', 'Close Miniplayer');
    closeBtn.title = 'Close Miniplayer';
    closeBtn.innerHTML = `<svg viewBox="0 0 16 16" width="12" height="12" fill="currentColor"><path d="M8 6.94 12.47 2.47l1.06 1.06L9.06 8l4.47 4.47-1.06 1.06L8 9.06l-4.47 4.47-1.06-1.06L6.94 8 2.47 3.53l1.06-1.06z"/></svg>`;
    ...
    Object.assign(canvasWrap.style, { flex: '1 1 auto', minHeight: '0', display: 'flex' });
    Object.assign(pipCanvas.style, { width: '100%', height: '100%', display: 'block' });
    canvasWrap.appendChild(pipCanvas);

    win.appendChild(header);
    win.appendChild(canvasWrap);

    // Dragging: pointer events cover both mouse and touch in one listener set.
    header.addEventListener('pointerdown', (e) => {
      header.style.cursor = 'grabbing';
      header.setPointerCapture(e.pointerId);
      startPagePipDrag(e.clientX, e.clientY);
    });
    header.addEventListener('pointermove', (e) => {
      if (!pagePipDragState) return;
      movePagePipDrag(e.clientX, e.clientY);
    });
    const endDrag = () => { header.style.cursor = 'grab'; endPagePipDrag(); };
    ...
    const onWindowResize = () => clampPagePipToViewport(win);
    win._resizeHandler = onWindowResize;
    window.addEventListener('resize', onWindowResize, { passive: true });
  }

  function hidePagePipWindow() {
    if (!pagePipWindow) return;
    if (pagePipWindow._resizeHandler) {
      window.removeEventListener('resize', pagePipWindow._resizeHandler);
    }
    // Move pipCanvas back out before the wrapper is removed, so the canvas
    // node itself (and its drawing context/state) survives for next time.
    document.body.appendChild(pipCanvas);
    Object.assign(pipCanvas.style, { position: 'fixed', left: '-9999px', top: '-9999px', width: '1px', height: '1px' });
    pagePipWindow.remove();
    pagePipWindow = null;
    pagePipDragState = null;
  }
    ...
    try {
      if (typeof pipVideo.requestPictureInPicture === 'function') {
        ...
        try {
          ...
          // Firefox for Android exposes requestPictureInPicture() as a
          // function everywhere, but the browser has no native PiP
          // implementation on Android and rejects with NotSupportedError.
          // Don't treat that as fatal - fall through to the next tier
          // instead of aborting the whole toggle here.
          console.info('📺 [Lyrics+ PiP] native requestPictureInPicture() unavailable on this platform, falling back:', nativeErr && nativeErr.name);
        }
      }
