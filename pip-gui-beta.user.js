...
//   single event, which felt abrupt and slightly jittery - especially on
//   touch, where touchmove fires far more densely than the display can
//   paint. They now only update a headerScrollTarget value; a single rAF
//   loop (headerScrollAnimationStep, via setHeaderScrollTarget) eases the
//   real scrollLeft toward that target each frame at a fixed rate
//   (HEADER_SCROLL_EASE). This coalesces bursts of touchmove/wheel events
...
// Softened tracking: pointer/wheel input used to snap buttonGroup.scrollLeft
// straight to the target on every single event, which felt abrupt and a bit
// jittery - especially on touch, where touchmove fires much more densely
// than a display can paint. Instead, pointer/wheel input now only updates a
// "target" scrollLeft, and one rAF loop eases the real scrollLeft toward it
// each frame. This smooths out touch drag and mouse drag alike, coalesces
// bursts of touchmove/wheel events into a single write per frame, and gives
// a small, pleasant "catch-up" settle once the pointer stops or lifts.
const HEADER_SCROLL_EASE = 0.35; // 0-1: higher = snappier, lower = softer
let headerScrollTarget = null;
let headerScrollRafId = null;

function headerScrollAnimationStep() {
  if (headerScrollTarget === null) { headerScrollRafId = null; return; }
  const current = buttonGroup.scrollLeft;
  const diff = headerScrollTarget - current;
  if (Math.abs(diff) < 0.5) {
    buttonGroup.scrollLeft = headerScrollTarget;
    // Keep the loop alive only while actively dragging (pointer might still
    // move again); once idle and settled, stop it entirely.
    if (!headerScrollDragging) {
      headerScrollTarget = null;
      headerScrollRafId = null;
      return;
    }
  } else {
    buttonGroup.scrollLeft = current + diff * HEADER_SCROLL_EASE;
  }
  headerScrollRafId = requestAnimationFrame(headerScrollAnimationStep);
}

function setHeaderScrollTarget(value) {
  const scrollableWidth = buttonGroup.scrollWidth - buttonGroup.clientWidth;
  headerScrollTarget = clamp(value, 0, Math.max(0, scrollableWidth));
  if (headerScrollRafId === null) {
    headerScrollRafId = requestAnimationFrame(headerScrollAnimationStep);
  }
}
...
function onHeaderScrollDragStart(e) {
  ...
  setHeaderScrollTarget(headerScrollPointerToScrollLeft(clientX));
}
...
function onHeaderWheel(e) {
  ...
  e.preventDefault();
  // Base off the in-flight target (if any) rather than the current
  // scrollLeft, so a quick burst of wheel ticks accumulates smoothly instead
  // of each tick fighting the still-easing position from the previous one.
  const base = headerScrollTarget !== null ? headerScrollTarget : buttonGroup.scrollLeft;
  setHeaderScrollTarget(base - e.deltaY);
}
