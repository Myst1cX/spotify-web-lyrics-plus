...
> 10. **Picture-in-Picture mode:** Turn the feature on to replace the lyrics container with a video element that streams a canvas rendering of the lyrics. This allows the PiP toggle to appear. Click the PiP toggle to move the lyrics into a floating Picture-in-Picture window while the original lyrics container receives a placeholder. Close the PiP window and toggle the feature off to restore the original lyrics container.  
