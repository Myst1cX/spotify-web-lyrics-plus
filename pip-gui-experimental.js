...
// Currently LRCLIB set as default provider when i open Lyrics. But I actually want it to be set to automatically open provider which has the lyrics. The order should be as follows. Prefer synced over unsynced. Prefer LRCLib > Kpoe > Genius order. Example song: SA Gi Tu by MIRA, rares (currently have lrclib as default provider so it loads lrc and says track not found, bc lrclib doesnt have this song's lyrics so i then had to manually switch to kpoe which had the lyrics. basically make it autodetect with the prefer order i told u, which provider to open first. hopefully this can be made into automatic thing without need for Loading.. for each b4 findging out which has.
