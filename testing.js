...
i just fucking did. only the snippet is what u need. async function fetchGeniusLyrics(info) {
  ...
  for (const fallbackQuery of fallbackQueries) {
    ...
    try {
      ...
      // === NEW: Detect translation URLs and try to replace them with original URLs if possible ===
      ...
      for (const hit of songHits) {
        ...
        if (isTranslationPage(result)) {
          artistScore -= 10;
          console.log(`[Genius] Penalizing translation/fan page: ${result.url}`);
        }
        ...
        if (isSimpleOriginalUrl(result.url)) {
          artistScore += 5;
          console.log(`[Genius] Boosting artist score: simple original URL detected`);
        }
      ...
      // Attempt to fetch additional artists from metadata (if available)
const metadataBlocks = [...doc.querySelectorAll('.SongHeader__Artist, .HeaderMetadata__Section, .HeaderMetadata__Unit, .metadata_unit-info')];
const extraArtistTokens = new Set();

for (const block of metadataBlocks) {
  const text = block.textContent?.trim();
  if (text && text.length > 1 && text.length < 100 && !text.includes('Embed')) {
    const normArtists = normalizeArtists(text);
    normArtists.forEach(token => extraArtistTokens.add(token));
  }
}

// Merge into resultArtists set
if (extraArtistTokens.size) {
  console.log(`[Genius] Extracted extra artists from metadata: ${Array.from(extraArtistTokens).join(', ')}`);
  extraArtistTokens.forEach(token => targetArtists.add(token));
}
  ...
  console.log("[Genius] Lyrics not found after trying all fallback queries");
