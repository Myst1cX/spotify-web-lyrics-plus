...
async function translateText(text, targetLang) {
  ...
  removeSongFeat(str) {
    // Remove "feat. ...", "ft. ...", etc. (non-greedy match)
    return str.replace(/\s*(?:feat\.?|ft\.?|featuring)\s+.*?(?=$|\s*[-–—])/i, '').trim();
  },
...
async function fetchGeniusLyrics(info) {
  ...
  const translationKeywords = [
    "translation", "übersetzung", "перевод", "çeviri", "çeviriler", "çevri", "traducción", "traduções", "traduction",
    "traductions", "traduzione", "traducciones", "traducciones-al-espanol", "fordítás", "fordítások", "tumaczenie",
  ...
  const normalizeArtists = artist => artist.toLowerCase().split(/,|&|feat|ft|and|\band\b/gi).map(s => normalize(s.trim())).filter(Boolean);
  const extractFeaturedArtistsFromTitle = title => {
    const matches = title.match(/\((?:feat\.?|ft\.?|with)\s+([^)]+)\)/i);
    return matches ? matches[1].split(/,|&|and/).map(s => normalize(s.trim())) : [];
  };
  const extractArtistsFromTitle = title => {
    const leftPart = title.split(" - ")[0];
    return leftPart ? leftPart.split(/,|&|feat\.?|ft\.?|and|\band\b/gi).map(s => normalize(s.trim())).filter(Boolean) : [];
  };
  ...
  function deriveOriginalUrlFromTranslation(translationUrl) {
    try {
  ...
  for (const fallbackQuery of fallbackQueries) {
    ...
    try {
      ...
      searchJson = JSON.parse(searchRes.responseText);
    } catch (e) {
      console.error("[Genius] Failed to search Genius:", e);
      continue;
    }
  ...
  // 1. If it's a non-translation direct lyrics page, fetch it!
  if (
    !isTranslationPage(result) &&
    result.url &&
    isSimpleOriginalUrl(result.url)
  ) {
    // Fetch lyrics HTML
    try {
  ...
  // 2. If it's a translation page, try to derive the original
  if (isTranslationPage(result)) {
    ...
    if (originalUrl) {
      ...
      console.log(`[Genius] Could not derive original URL from translation URL: ${result.url}`);
    }
  }
}
...
const ProviderGenius = {
  async findLyrics(info) {
    ...
    try {
      ...
      console.error("[Genius] findLyrics exception:", e);
      return { error: e.message };
    }
  },
  getUnsynced(body) {
    if (!body?.plainLyrics) {
      console.log("[Genius] getUnsynced: no plainLyrics");
      return null;
    }
    return parseGeniusLyrics(body.plainLyrics).unsynced;
  },
  getSynced() {
    console.log("[Genius] getSynced called - returning null (not supported)");
    return null;
  },
};
