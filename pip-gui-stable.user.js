...
// updateProgressUIFromSpotify()'s visible-text source is unchanged (it already
// matched true position). Its native-range-input and --progress-bar-transform-percent
// fallbacks (used only when the text isn't available) are patched with the same
// -1000ms compensation rather than removed. seekTo()'s range-input write now
...
// Its CSS-progress-bar-handle click-simulation fallback (only used if the range
// input can't be found at all) is unchanged - there's no DOM evidence either way
// that this path is subject to the same offset, so it's left as-is pending an
// actual test of that fallback specifically.
...
`;
  ...
  function addButton(maxRetries = LIMITS.BUTTON_ADD_MAX_RETRIES) {
    ...
    if (!micBtn) {
      ...
      if (stray) {
        stray.remove();
        DEBUG.info('Button', 'Lyrics+ button removed - no visible mic button in current player (podcast?)');
      }
